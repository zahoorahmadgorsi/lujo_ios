...
extension GoLujoAPIManager  {
    ...
    func search(_ token: String, term: String?, cityId: Int?, currentLocation: CLLocation?, completion: @escaping ([Restaurants]?, Error?) -> Void) {
        Alamofire.request(DiningRouter.search(token, term, cityId, currentLocation?.coordinate.latitude, currentLocation?.coordinate.longitude))
            .responseJSON { response in
                ...
                case 1 ... 199: // Transfer protoco-level information: Unexpected
                    completion(nil, self.handleError(response, statusCode))
                case 200 ... 299: // Success
                    guard let result = try? JSONDecoder().decode(LujoServerResponse<[Restaurants]>.self,
                                                                 from: response.data!)
                    else {
                        completion(nil, BackendError.parsing(reason: "Unable to parse response"))
                        return
    ...
    func geopoint(token: String, type: String, latitude: Float, longitude: Float, radius: Int, completion: @escaping ([Restaurants]?, Error?) -> Void) {
