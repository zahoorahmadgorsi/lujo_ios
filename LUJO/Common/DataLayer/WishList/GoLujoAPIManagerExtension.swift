...
extension GoLujoAPIManager  {
    ...
    func getUnReadPushNotificationsCount( completion: @escaping (Int?, Error?) -> Void) {
        Alamofire.request(WishListRouter.getUnReadPushNotificationsCount)
            .responseJSON { response in
                guard response.result.error == nil else {
                    completion(nil, response.result.error!)
                    return
                ...
                case 1 ... 199: // Transfer protoco-level information: Unexpected
                    completion(nil, self.handleError(response, statusCode))
                case 200 ... 299: // Success
                    guard let result = try? JSONDecoder().decode(LujoServerResponse<Int>.self,
                                                                 from: response.data!)
                    else {
                        completion(nil, BackendError.parsing(reason: "Unable to parse response"))
                        return
        ...
        Alamofire.request(WishListRouter.getPushNotifications(pageSize,type))
            .responseJSON { response in
                ...
                case 1 ... 199: // Transfer protoco-level information: Unexpected
                    completion(nil, self.handleError(response, statusCode))
                case 200 ... 299: // Success
                    guard let result = try? JSONDecoder().decode(LujoServerResponse<[PushNotification]>.self,
                                                                 from: response.data!)
                    else {
                        completion(nil, BackendError.parsing(reason: "Unable to parse response"))
                        return
    ...
    func deletePushNotifications(id:String, completion: @escaping (String?, Error?) -> Void) {
        Alamofire.request(WishListRouter.deletePushNotifications(id))
            .responseJSON { response in
                guard response.result.error == nil else {
                    completion(nil, response.result.error!)
                    return
    ...
    func readPushNotifications(id:String, completion: @escaping (String?, Error?) -> Void) {
        Alamofire.request(WishListRouter.readPushNotifications(id))
            .responseJSON { response in
                guard response.result.error == nil else {
                    completion(nil, response.result.error!)
                    return
    ...
    func getBrands(completion: @escaping ([Taxonomy]?, Error?) -> Void) {
        Alamofire.request(WishListRouter.getBrands)
            .responseJSON { response in
    ...
    func getCategories(completion: @escaping ([Taxonomy]?, Error?) -> Void) {
        Alamofire.request(WishListRouter.getCategories)
            .responseJSON { response in
                print("Request URL: \(String(describing: response.request)) \nRequest Body: \(String(data: response.request?.httpBody ?? Data(), encoding: .utf8)!) \nResponse Body: \(String(data: response.data ?? Data(), encoding: .utf8)!)")
    ...
    func getColors(completion: @escaping ([Taxonomy]?, Error?) -> Void) {
        Alamofire.request(WishListRouter.getColors)
            .responseJSON { response in
                print("Request URL: \(String(describing: response.request)) \nRequest Body: \(String(data: response.request?.httpBody ?? Data(), encoding: .utf8)!) \nResponse Body: \(String(data: response.data ?? Data(), encoding: .utf8)!)")
