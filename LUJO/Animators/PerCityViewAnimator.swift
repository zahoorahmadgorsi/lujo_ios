...
final class PerCityViewAnimator: NSObject, UIViewControllerAnimatedTransitioning {

    private let type: PresentationType
    private let firstViewController: PerCityViewController
...
//  Important note: if something “goes wrong”, for example, you can’t prepare all the needed properties (basically the init fails), make sure to return nil. This way the app will use default present/dismiss animation and the user won’t be stuck somewhere in the middle of the transition.
    init?(type: PresentationType, firstViewController: PerCityViewController, secondViewController: ProductDetailsViewController, selectedCellImageViewSnapshot: UIView) {
        ...
        guard let window = firstViewController.view.window ?? secondViewController.view.window,
            let selectedCell = firstViewController.selectedImageView
...
//        controllerImageSnapshot — snapshot of the image of the 2nd VC.
        guard let selectedCell = firstViewController.selectedImageView,
